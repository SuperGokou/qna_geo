...
function Tasks({
  ...
  return <><div className="page-heading"><div><div className="eyebrow">签约前核查</div><h1>待核实事项</h1><p>将关键缺口落实到资料、演示与书面条款。</p></div><IconButton icon={Download} label="导出核实清单" onClick={exportTasks} /></div><div className="task-summary"><div><strong>{done.length}<span>/ {tasks.length}</span></strong><span>已跟进</span></div><p>勾选仅记录本机跟进进度，不改变资料的证据状态。</p></div><div className="filter-tabs">{['全部', '待跟进', '已跟进'].map(f => <button key={f} className={filter === f ? 'active' : ''} onClick={() => setFilter(f)}>{f}</button>)}</div><div className="task-list">{tasks.filter(([id]) => filter === '全部' || (filter === '已跟进' ? done.includes(id) : !done.includes(id))).map(([id, title, body, owner]) => <label className={`task ${done.includes(id) ? 'done' : ''}`} key={id}><input type="checkbox" checked={done.includes(id)} onChange={() => toggle(id)} /><div><h3>{title}</h3><p>{body}</p></div><span>{owner}</span></label>)}</div><section className="guardrail"><ShieldCheck size={21} /><div><h3>扩大投入之前</h3><p>原始交付可复算、客户愿意续费、贡献毛利覆盖获客成本。仅有推荐率上涨，不构成扩张依据。</p></div></section></>;
}
function Chat({
  open,
  onClose,
  ...
  return <aside className={`chat-panel ${open ? 'open' : ''}`} aria-label="决策助手">
    <div className="chat-header"><span className="bot-symbol"><MessageSquare size={19} /></span><div><strong>项目问答</strong><span><i className={status?.connected ? 'configured' : ''} />{status?.connected ? 'DeepSeek · 已配置' : '本地资料模式'}</span></div><IconButton icon={Plus} label="新建对话" onClick={clear} disabled={busy} /><IconButton icon={PanelRightClose} label="关闭助手" className="icon-button mobile-chat" onClick={onClose} /></div>
    ...
    <div className="chat-body" aria-live="polite">
      {!messages.length && <div className="chat-welcome"><h2>肖总，您好。</h2><p>这次合作，有几个问题值得先谈。</p><div className="suggestions">{suggestions.map(q => <button key={q} onClick={() => {
            setInput(q);
...
function Login({
  ...
  const [error, setError] = useState('');
  const [busy, setBusy] = useState(false);
  return <main className="login"><div className="login-content"><span className="brand-mark">G</span><div className="eyebrow">GEO / 区域合作研究</div><h1>进入决策工作台</h1><p>扬州 / 泰州区域合作研究</p><form onSubmit={async e => {
        e.preventDefault();
        setBusy(true);
        try {
          await api('/api/login', {
